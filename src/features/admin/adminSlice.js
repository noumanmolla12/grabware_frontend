...
//   async ({ username, password }, { rejectWithValue }) => {
//     try {
//       return await loginAdmin({ username, password }); // 👈 correct field names
//     } catch (error) {
//       return rejectWithValue(error.response?.data?.msg || 'Login failed');
//     }
// });
...
//   const admin = action.payload.admin; // 👈 Extract only admin
//   state.currentAdmin = admin;
//   localStorage.setItem('admin', JSON.stringify(admin)); // 👈 Store clean object
// })
